/*
	Code taken from https://github.com/jamolnng/argparse
*/
...
namespace argparse {
	namespace detail {
		...
		static std::string _escape(const std::string &in) {
			...
			if (in.find(' ') != std::string::npos)
				out = std::string(_T("\"")).append(out).append(_T("\""));
			return out;
		}
	...
	public:
		...
		void print_help() {
			std::cout << _T("Usage: ") << _bin;
			if (_positional_arguments.empty()) {
				std::cout << _T(" [options...]") << std::endl;
			}
			else {
				int current = 0;
				for (auto &v : _positional_arguments) {
					if (v.first != Argument::Position::LAST) {
						for (; current < v.first; ++current) {
							std::cout << _T(" [") << current << _T("]");
						}
						std::cout << _T(" [")
							<< detail::_ltrim_copy(
								_arguments[static_cast<size_t>(v.second)]._names[0],
								[](int c) -> bool {
							return c != static_cast<int>(_T('-'));
						})
							<< _T("]");
					}
					else {
			...
			for (auto &a : _arguments) {
				...
				for (size_t n = 1; n < a._names.size(); ++n) {
					name.append(_T(", ") + a._names[n]);
				}
				std::cout << _T("    ") << std::setw(23) << std::left << name << std::setw(23)
					<< a._desc;
				if (a._required) {
					std::cout << _T(" (Required)");
				}
				std::cout << std::endl;
			}
		}

		Result parse(int argc, const TCHAR *argv[]) {
			Result err;
			if (argc > 1) {
				// build name map
				for (auto &a : _arguments) {
					for (auto &n : a._names) {
						std::string name = detail::_ltrim_copy(
							n, [](int c) -> bool { return c != static_cast<int>(_T('-')); });
						if (_name_map.find(name) != _name_map.end()) {
							return Result(_T("Duplicate of argument name: ") + n);
						}
			...
			if (_help_enabled && exists(_T("help"))) {
				return Result();
			}
			err = _end_argument();
			if (err) {
			...
			for (auto &a : _arguments) {
				if (a._required && !a._found) {
					return Result(_T("Required argument not found: ") + a._names[0]);
				}
				if (a._position >= 0 && argc >= a._position && !a._found) {
					return Result(_T("Argument ") + a._names[0] + _T(" expected in position ") +
		...
		void enable_help() {
			add_argument(_T("-h"), _T("--help"), _T("Shows this page"), false);
			_help_enabled = true;
		}
	...
	private:
		Result _begin_argument(const std::string &arg, bool longarg, int position) {
			...
			if (_current != -1) {
				return Result(_T("Current argument left open"));
			}
			...
			if (longarg) {
				...
				if (nmf == _name_map.end()) {
					return Result(_T("Unrecognized command line option '") + arg_name + _T("'"));
				}
				_current = nmf->second;
				...
				if (equal_pos == 0 ||
					...
					return Result(_T("Malformed argument: ") + arg);
				}
				else if (equal_pos > 0) {
		...
		Result _end_argument() {
			if (_current >= 0) {
				...
				if (static_cast<int>(a._values.size()) < a._count) {
					return Result(_T("Too few arguments given for ") + a._names[0]);
				}
				if (a._count >= 0) {
					if (static_cast<int>(a._values.size()) > a._count) {
						return Result(_T("Too many arguments given for ") + a._names[0]);
					}
				}
			}
			return Result();
